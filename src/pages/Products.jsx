...
const Products = () => {
  ...
  const obtenerProductos = async () =>{
    ...
    if (categoriaBuscar === 'Más vendidos') {
  ...
  useEffect(()=>{
    setCategoriaBuscar('Más vendidos')
  },[])
  ...
  return (
    ...
    <main className='grid grid-cols-5 px-5 py-5 gap-5'>
      ...
      <section className='col-span-4'>
        <div className='text-3xl bg-[#17823C] text-white text-center py-5 font-semibold underline'>{categoriaBuscar}</div>
        <section className='grid grid-cols-4 gap-10 mt-7'>
          {productos.map((producto) =>(
          <div key={producto.id} className='w-[16.875rem] h-[25rem] items-center flex flex-col justify-center gap-3 p-3 bg-[#149968] text-white font-semibold rounded-2xl'>
            ...
            <div className='rounded-xl bg-[#138F84] p-1 px-4 text-white'>
              <h3>Precio: S/ {(producto.Precio).toFixed(2)}</h3>
            </div>
          </div>)
          )}
        </section>
        <section className='flex justify-center items-center mt-5 p-5 gap-5'>
          {Array.from({ length: pages }, (_, index) => (
            <button  key={index} className={`p-2 text-xl font-medium rounded-xl ${
              index + 1 === pageList ? 'bg-[#149968] text-white' : 'bg-[#138F84] text-gray-700'
